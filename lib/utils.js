...
function getNewPath(origPath, callback, i) {
    var dir = path.dirname(origPath),
        oldName = path.basename(origPath),
        newName, joined;
    if (typeof i === "undefined") i = 1;
    if (i === 1) {
        joined = path.join(dir, oldName);
    } else {
        if (oldName[0] === ".")
            newName = path.basename(origPath).replace(/^(.+$)/, "$1-" + i);
        else
            newName = path.basename(origPath).replace(/^([^\.]+)/, "$1-" + i);
        joined = path.join(dir, newName);
    }
    fs.stat(joined, function (error) {
        if (error) {
            console.log("JO", joined);
            callback(joined); // File is (probably) save to write
        }
        else
            getNewPath(origPath, callback, i + 1);
    });
}
