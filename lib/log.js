...
function response(req, res) {
    var responseTime = "";
    if (req.time)
        responseTime = "[" +  color.magenta + (Date.now() - req.time) + "ms" + color.reset + "]";

    log(
        color.cyan, req.socket.remoteAddress, color.reset, ":",
        color.blue, req.socket.remotePort, color.reset, " ",
        req.method.toUpperCase(), " ", decodeURIComponent(req.url), " ", res.statusCode, " ",
        responseTime, color.reset
    );
}
...
function socket(ip, port) {
    return [color.cyan, ip, color.reset, ":", color.blue, port, color.reset].join("");
}
