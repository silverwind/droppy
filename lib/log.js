...
var log = function log(req, res) {
    ...
    if (req && req.method) elems.push(chalk.yellow(req.method.toUpperCase()) + " ");
    if (req && req.url) elems.push(decodeURIComponent(req.url) + " ");
    if (res && res.statusCode) {
        ...
        elems.push(statusCode + " ");
    }
    if (req && req.time) elems.push("[" +  chalk.magenta((Date.now() - req.time) + "ms") + "]");
...
log.simple = function () {
    console.log(chalk.yellow(" ->> ") + Array.prototype.slice.call(arguments, 0).join(""));
};

log.error = function error(err) {
    if (err === undefined) return console.log(chalk.red("Error handler called without an argument"));
