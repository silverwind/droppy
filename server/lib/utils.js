...
utils.resolve = function resolve(str) {
    if (/^~/.test(str)) {
        if (process.platform === "win32") {
            var home = process.env.USERPROFILE || process.env.HOMEDRIVE + process.env.HOMEPATH;
...
utils.tlsInit = function tlsInit(keyPath, certPath, caPath, callback) {
    ...
    async.parallel([
        ...
        function (cb) {
            fs.stat(certPath, function (err, stats) {
                if (err && err.code === "ENOENT") {
                    fs.writeFile(certPath, cert, function (err) {
                        cb(err, cert);
                    });
                } else if (stats.isFile()) {
                    fs.readFile(certPath, function (err, data) {
                        cb(err, String(data));
                    });
                }
            });
        ...
        function (cb) {
            fs.stat(caPath, function (err, stats) {
                if (err && err.code === "ENOENT") {
